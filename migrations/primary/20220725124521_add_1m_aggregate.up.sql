-- Chỉ tạo view nếu table và extension tồn tại
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'fieldkit' AND tablename = 'sensor_data') 
       AND EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'timescaledb') THEN
        CREATE MATERIALIZED VIEW IF NOT EXISTS fieldkit.sensor_data_1m
